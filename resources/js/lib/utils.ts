...
export function validateFormTitle(title: string): string | null {
  if (!title.trim()) return "Form title is required"
  if (title.length < 3) return "Form title must be at least 3 characters"
  if (title.length > 100) return "Form title must be less than 100 characters"
  return null
}

export function validateSectionName(name: string): string | null {
  if (!name.trim()) return "Section name is required"
  if (name.length < 2) return "Section name must be at least 2 characters"
  if (name.length > 50) return "Section name must be less than 50 characters"
  return null
}

export function validateFieldLabel(label: string): string | null {
  if (!label.trim()) return "Field label is required"
  if (label.length < 2) return "Field label must be at least 2 characters"
  if (label.length > 100) return "Field label must be less than 100 characters"
  return null
}

export function validateFieldOptions(options: string[] | undefined, fieldType: string): string | null {
  if (!["select", "radio", "checkbox"].includes(fieldType)) return null

  if (!options || options.length === 0) {
    return "Field must have at least one option"
  }

  const validOptions = options.filter((opt) => opt.trim())
  if (validOptions.length === 0) {
    return "Field must have at least one valid option"
  }

  if (validOptions.length !== new Set(validOptions).size) {
    return "Field options must be unique"
  }
...
export function getErrorBorderClass(hasError: boolean, isSelected = false): string {
  if (hasError && isSelected) return "border-red-500 bg-red-100"
  if (hasError) return "border-red-300 bg-red-50 hover:border-red-400"
