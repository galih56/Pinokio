...
export const IssuesList = ({ onIssuePrefetch }: IssuesListProps) => {
  ...
  const columns: ColumnDef<Issue>[] = [
    ...
    },
    {
      accessorKey: "title",
      header: "Title",
      meta: { cellClassName: "min-w-[200px] max-w-[300px]" }, // Better mobile sizing
      cell: ({ row }) => {
        const issue = row.original
    ...
    },
    {
      ...
      header: "Description",
      meta: { cellClassName: "min-w-[250px] max-w-[400px]" }, // Better mobile sizing
      cell: ({ row }) => {
        const issue = row.original
        let email = ""
        let name = ""
        ...
        const expanded = expandedStates[issue.id] || false
        ...
        return (
          <div className="w-full">
            ...
            {name && email && (
              <p className="text-xs">
                Created By : {name} <span className="text-xs text-gray-600">{email}</span>
              </p>
            )}
            {sanitizedContent.length > 200 && (
              <Button
                ...
                onClick={() => setExpandedStates({ ...expandedStates, [issue.id]: !expanded })}
              >
